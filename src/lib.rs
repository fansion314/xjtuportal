...
pub async fn run(config: AppConfig, config_path: Option<PathBuf>) -> Result<RunStatus> {
    ...
    for targets in target_groups {
        ...
        tasks.spawn(async move { run_target_group(config, config_path, targets).await });
    }
...
pub async fn run_default_login(config: AppConfig, config_path: Option<PathBuf>) -> Result<()> {
    ...
    run_target(&config, config_path.as_deref(), &target).await
}
...
pub async fn list_default_sessions(
    ...
    let (client, token) = default_session_client(&config, config_path.as_deref()).await?;
...
async fn default_session_client(
    ...
    let token = login_for_session_token(config, config_path, &target, &client).await?;
    Ok((client, token))
}
...
async fn login_for_session_token(
    ...
    let redirect_url = match client.check_network().await? {
        ...
        NetworkStatus::Redirected(redirect_url) => {
            update_nas_ip_from_redirect(config, config_path, &redirect_url);
            redirect_url
        }
    };
...
fn update_nas_ip_from_redirect(config: &AppConfig, config_path: Option<&Path>, redirect_url: &str) {
    ...
    if config.network.nas_ip.as_deref() == Some(nas_ip.as_str()) {
        return;
    }

    match write_network_nas_ip(config_path, &nas_ip) {
        Ok(true) => info!(
